...
export default function Course({ course }: { course: Course }) {
  ...
  const top = (course.start_time - 480) / 60 * 4;
  const height = duration / 60 * 4;
  const teacherName = course.tutor ? TEACHERS_MAP.get(course.tutor) || course.tutor : 'N/A';

  return (
    <div
      className="absolute w-full p-2 rounded-lg shadow-md flex flex-col text-center justify-center"
      style={{
        top: `${top}rem`,
        height: `${height}rem`,
    ...
    >
      <div className="text-sm font-bold">{course.course.module.name}</div>
      <div className="text-xs">Salle: {course.room.name}</div>
      <div className="text-xs">Prof: {teacherName}</div>
    </div>
  );
}